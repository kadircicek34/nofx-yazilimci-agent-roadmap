...
## OpenBB Prompt Enrichment

NOFX can enrich the existing LLM prompt with **OpenBB-backed market context** while preserving the original prompt structure.

What gets added:
- **Cross-asset macro regime** from OpenBB/yfinance benchmarks such as `SPY`, `QQQ`, `DX-Y.NYB`, `GLD`, `CL=F`, and `^VIX`
- **Coin-specific technical context** derived from OpenBB historical price data (e.g. returns, RSI, MACD, SMA/EMA, volatility)
- **Coin-specific micro-news + lightweight sentiment** derived from OpenBB-fetched recent articles

Strategy config defaults now enable OpenBB enrichment with sensible guardrails:
- `enable_openbb=true`
- `enable_openbb_macro=true`
- `enable_openbb_news=true`
- `enable_openbb_technical=true`
- `openbb_news_limit=4`
- `openbb_history_days=90`
- `openbb_timeout_seconds=45`
- `openbb_cache_ttl_seconds=900`

Behavior notes:
- OpenBB enrichment is **best-effort**. If OpenBB is unavailable or a provider call fails, NOFX continues running and omits the affected OpenBB sections.
- OpenBB requests are **cached** to avoid repeatedly hammering Python/OpenBB on every decision loop.
- The original NOFX prompt layout remains intact; OpenBB adds a labeled macro section plus labeled per-symbol enrichment sections.

## Security Defaults

This fork also tightens several defaults for safer private deployment:
- `TRANSPORT_ENCRYPTION=true` by default
- wildcard CORS replaced with `CORS_ALLOWED_ORIGINS`
- unauthenticated public password reset disabled by default (`ENABLE_PUBLIC_PASSWORD_RESET=false`)
- secret-bearing AI/exchange config logs are redacted
- position opens now attempt a **fail-safe rollback** if stop-loss / take-profit protection cannot be installed after entry
