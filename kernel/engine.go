...
type Context struct {
	...
	OpenBBData         *openbbprovider.EnrichmentResponse `json:"-"`
...
type StrategyEngine struct {
	...
	openbbClient *openbbprovider.Client
}
...
func NewStrategyEngine(config *store.StrategyConfig) *StrategyEngine {
	...
	openbbClient := openbbprovider.NewClient(openbbprovider.Config{
		NewsLimit:    config.Indicators.OpenBBNewsLimit,
		HistoryDays:  config.Indicators.OpenBBHistoryDays,
		Timeout:      time.Duration(config.Indicators.OpenBBTimeoutSeconds) * time.Second,
		CacheTTL:     time.Duration(config.Indicators.OpenBBCacheTTLSeconds) * time.Second,
		MacroSymbols: config.Indicators.OpenBBMacroSymbols,
		IncludeMacro: config.Indicators.EnableOpenBBMacro,
		IncludeNews:  config.Indicators.EnableOpenBBNews,
		IncludeTech:  config.Indicators.EnableOpenBBTechnical,
	})

	return &StrategyEngine{
		config:       config,
		nofxosClient: client,
		openbbClient: openbbClient,
	}
}

func applyOpenBBDefaults(config *store.StrategyConfig) {
	if config == nil {
		return
	}
	indicators := &config.Indicators
	legacyOpenBBUnset := !indicators.EnableOpenBB && !indicators.EnableOpenBBMacro && !indicators.EnableOpenBBNews && !indicators.EnableOpenBBTechnical &&
		indicators.OpenBBNewsLimit == 0 && indicators.OpenBBHistoryDays == 0 && indicators.OpenBBTimeoutSeconds == 0 && indicators.OpenBBCacheTTLSeconds == 0 && len(indicators.OpenBBMacroSymbols) == 0
	if !legacyOpenBBUnset {
		return
	}

	defaults := store.GetDefaultStrategyConfig(config.Language)
	indicators.EnableOpenBB = defaults.Indicators.EnableOpenBB
	indicators.EnableOpenBBMacro = defaults.Indicators.EnableOpenBBMacro
	indicators.EnableOpenBBNews = defaults.Indicators.EnableOpenBBNews
	indicators.EnableOpenBBTechnical = defaults.Indicators.EnableOpenBBTechnical
	indicators.OpenBBNewsLimit = defaults.Indicators.OpenBBNewsLimit
	indicators.OpenBBHistoryDays = defaults.Indicators.OpenBBHistoryDays
	indicators.OpenBBTimeoutSeconds = defaults.Indicators.OpenBBTimeoutSeconds
	indicators.OpenBBCacheTTLSeconds = defaults.Indicators.OpenBBCacheTTLSeconds
	indicators.OpenBBMacroSymbols = defaults.Indicators.OpenBBMacroSymbols
}
