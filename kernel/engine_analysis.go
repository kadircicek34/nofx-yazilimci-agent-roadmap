...
func GetFullDecisionWithStrategy(ctx *Context, mcpClient mcp.AIClient, engine *StrategyEngine, variant string) (*FullDecision, error) {
	...
	if err := fetchOpenBBEnrichment(ctx, engine); err != nil {
		logger.Infof("⚠️ OpenBB enrichment skipped: %v", err)
	}
...
func fetchOpenBBEnrichment(ctx *Context, engine *StrategyEngine) error {
	if ctx == nil || engine == nil || engine.openbbClient == nil {
		return nil
	}
	cfg := engine.GetConfig().Indicators
	if !cfg.EnableOpenBB {
		return nil
	}
	...
	resp, err := engine.openbbClient.Enrich(context.Background(), openbbprovider.EnrichmentRequest{
		Symbols:          symbols,
		MacroSymbols:     cfg.OpenBBMacroSymbols,
		NewsLimit:        cfg.OpenBBNewsLimit,
		HistoryDays:      cfg.OpenBBHistoryDays,
		TimeoutSeconds:   cfg.OpenBBTimeoutSeconds,
		IncludeMacro:     cfg.EnableOpenBBMacro,
		IncludeNews:      cfg.EnableOpenBBNews,
		IncludeTechnical: cfg.EnableOpenBBTechnical,
	})
	if err != nil {
		return err
	}
