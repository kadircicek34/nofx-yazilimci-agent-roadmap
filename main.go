...
func main() {
	...
	if cfg.JWTSecret == "default-jwt-secret-change-in-production" {
		logger.Info("⚠️ Using the default JWT secret. Set JWT_SECRET before exposing this instance to untrusted networks.")
	}
	if !cfg.TransportEncryption {
		logger.Info("⚠️ Transport encryption is disabled. Sensitive config updates will travel in plaintext unless you enable TRANSPORT_ENCRYPTION=true.")
	}
